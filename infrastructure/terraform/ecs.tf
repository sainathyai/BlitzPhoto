...
# ECS Task Definition (will be updated via deployment script)
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([
    {
      name  = "backend"
      image = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/${var.project_name}-backend-${var.environment}:latest"
      ...
      environment = [
        ...
        },
        {
          ...
          value = "http://localhost:5173,http://localhost:5174,http://localhost:3000,http://localhost:19006,http://sainathyai-frontend-dev.s3-website-us-west-2.amazonaws.com,https://blitzphoto.sainathyai.com"
        }
      ]
...
resource "aws_secretsmanager_secret_version" "jwt_secret" {
  ...
  secret_string = "dev-secret-key-blitzphoto-2025-change-in-production-${var.environment}"
}
